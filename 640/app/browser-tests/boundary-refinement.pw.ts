...
test("direct 2001 landing leads with its heading and keeps the compact newer control above the content viewport", async ({ page }) => {
  ...
  await expect(page.locator('.year-selector [aria-current="true"]')).not.toBeVisible();
...
test("sticky year and album context replace inline headings instead of duplicating them", async ({ page }) => {
  ...
  await expect(page.locator('.year-selector [aria-current="true"]')).toBeVisible();
  ...
  await expect(page.locator(".album-context__year")).toHaveCount(0);
});

test("compact year menu uses one unified jump history action", async ({ page }) => {
